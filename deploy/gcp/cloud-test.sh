...
# output. Token metering (tokens_total) is a NATIVE-agentd feature: nutrition-go
# reports it; the external-SDK contract shims (OpenAI/Claude SDK agents) return
# tokens_total=null by design, so we require tokens>0 only for native agents and
# merely report the count for SDK agents.
run_session() {  # $1=agent  $2=prompt(json-string)  $3=require_tokens(yes|no)
  local a="$1" prompt="$2" require_tokens="$3" sid text st toks
  ...
  if [ "$require_tokens" = yes ]; then
    if [ "${toks:-0}" -gt 0 ] 2>/dev/null; then
      pass "e2e $a: completed, ${#text} chars, tokens=$toks (native metering)"
    else
      fail "e2e $a: completed with output but native metering reported tokens=$toks (session $sid)"
    fi
  else
    # SDK-contract agent: tokens_total is null by design — report, don't gate.
    pass "e2e $a: completed, ${#text} chars (SDK agent; tokens not metered by design)"
  fi
}

#            agent               prompt                                                        require_tokens
run_session nutrition-go       '"Is plain water a healthy drink? Answer in one sentence."'    yes
run_session nutrition-openai   '"Is orange juice healthy? Answer in one sentence."'           no
run_session hello-claude       '"What makes a drink healthier? One sentence."'                no
run_session food-label-advisor '"Is 6g sugar per 100ml high for a drink? One sentence."'      no
...
skip "alertmanager — not part of the cloud compose"
