...
func TestFanoutMergesHealthyAgents(t *testing.T) {
	...
	for _, agent := range []string{"alpha", "beta"} {
		if !strings.Contains(body, fmt.Sprintf(`agent_turns_total{agent=%q,outcome="completed",replica="0"} 3`, agent)) {
	...
	if v := testutil.ToFloat64(c.agentUp.WithLabelValues("alpha", "0")); v != 1 {
...
func TestFanoutSkipsHangingAgent(t *testing.T) {
	...
	if v := testutil.ToFloat64(c.agentUp.WithLabelValues("hung", "0")); v != 0 {
		t.Fatalf("hung up = %v, want 0", v)
	}
	if v := testutil.ToFloat64(c.scrapeSkips.WithLabelValues("hung", "0", "timeout")); v != 1 {
...
func TestFanoutSkipsMalformedExposition(t *testing.T) {
	...
	if v := testutil.ToFloat64(c.agentUp.WithLabelValues("bad", "0")); v != 0 {
		t.Fatalf("bad up = %v, want 0", v)
	}
	if v := testutil.ToFloat64(c.scrapeSkips.WithLabelValues("bad", "0", "parse")); v != 1 {
...
func TestFanout404IsNoMetricsButStillUp(t *testing.T) {
	...
	_ = scrapeHandler(t, h)
	if v := testutil.ToFloat64(c.agentUp.WithLabelValues("shim", "0")); v != 1 {
		t.Fatalf("shim up = %v, want 1 (404 proves serving)", v)
	}
	if v := testutil.ToFloat64(c.scrapeSkips.WithLabelValues("shim", "0", "no_metrics")); v != 1 {
...
func TestFanoutEnforcesAgentLabel(t *testing.T) {
	...
	body := scrapeHandler(t, h)
	mustParseClean(t, body)
	if !strings.Contains(body, `agent_turns_total{agent="honest",outcome="completed",replica="0"} 3`) {
...
`+exposition("sneaky"))
	...
	if !strings.Contains(body, `runtime_agent_up{agent="sneaky",replica="0"} 1`) {
		t.Fatalf("control registry's own runtime_agent_up missing:\n%s", body)
	}
	// Legitimate (non-reserved) families from the same agent still merge.
	if !strings.Contains(body, `agent_turns_total{agent="sneaky",outcome="completed",replica="0"} 3`) {
		t.Fatalf("agent's legitimate family lost:\n%s", body)
	}
	if v := testutil.ToFloat64(c.scrapeSkips.WithLabelValues("sneaky", "0", "reserved_name")); v != 1 {
...
func TestFanoutTypeConflictDropsFamily(t *testing.T) {
	...
	skipOne := testutil.ToFloat64(c.scrapeSkips.WithLabelValues("one", "0", "type_conflict"))
	skipTwo := testutil.ToFloat64(c.scrapeSkips.WithLabelValues("two", "0", "type_conflict"))
	...
	case skipTwo == 1: // agent one's counter survived
		if !strings.Contains(body, "# TYPE flaky_metric counter") ||
			!strings.Contains(body, `flaky_metric{agent="one",replica="0"} 1`) {
	...
	case skipOne == 1: // agent two's gauge survived
		if !strings.Contains(body, "# TYPE flaky_metric gauge") ||
			!strings.Contains(body, `flaky_metric{agent="two",replica="0"} 2`) {
...
func TestFanout_RemoteTargetUsesBaseURLAndToken(t *testing.T) {
	...
	body := scrapeHandler(t, h)
	mustParseClean(t, body)
	if !strings.Contains(body, `agent_turns_total{agent="remote",outcome="completed",replica="0"} 3`) {
