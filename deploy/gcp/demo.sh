...
run_label() {  # $1=agent  $2=message
  ...
  sid=$(curl -s "$BASE/agents/$agent/sessions" -d "{\"message\":$2}" | jq -r .session_id)
  printf '\033[2m> session %s — streaming verdict:\033[0m\n\n' "$sid"
  curl -sN "$BASE/agents/$agent/sessions/$sid/stream?since=0" \
    | sed -n 's/^data: //p' \
    | jq -r 'select(.type=="text").text'
}

clear
say "1. The deployment: a control plane + three agents on separate machines"
echo "Three agents, in DIFFERENT languages/SDKs, attached to one control plane:"
curl -s "$BASE/agents" | jq -r '.[] | "  - \(.id)  (\(.name))  healthy=\(.healthy)"'
echo
echo "nutrition-go     -> native Go agent       (VM B + Postgres)"
echo "nutrition-openai -> OpenAI Agents SDK      (VM C + SQLite)"
echo "hello-claude     -> Claude Agent SDK       (VM C, 2nd container; dynamic agent)"
pause
...
say "4. A third agent (Claude SDK) on the same control plane — registered dynamically"
